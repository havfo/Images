 ...
 */
public class CannyEdgeFilter extends AbstractFilter {
	...
	public CannyEdgeFilter(BufferedImage image, View parent) {
		super(image, parent);
		lowThreshold = 2.5f;
		highThreshold = 7.5f;
	...
	private void readLuminance() {
		...
		if (type == BufferedImage.TYPE_INT_RGB
			...
			int[] pixels = (int[]) getImage().getData().getDataElements(0, 0,
					width, height, null);
			for (int i = 0; i < picsize; i++) {
				int p = pixels[i];
				int r = (p & 0xff0000) >> 16;
			...
			byte[] pixels = (byte[]) getImage().getData().getDataElements(0,
					0, width, height, null);
			for (int i = 0; i < picsize; i++) {
				data[i] = (pixels[i] & 0xff);
			}
		} else if (type == BufferedImage.TYPE_USHORT_GRAY) {
			short[] pixels = (short[]) getImage().getData().getDataElements(0,
					0, width, height, null);
			for (int i = 0; i < picsize; i++) {
